...
module.exports.run = async function({ api, event, Users, Threads, Currencies }) {
  ...
  const ADMINBOT = api.getCurrentUserID();
  if (mention == ADMINBOT) return api.sendMessage("Định trộm cả Admin à -.-", threadID, messageID);
  ...
  var tile = Math.floor(Math.random() * 10000) + 1;
  if (tile < cc) {
