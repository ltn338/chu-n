...
if (typeof data.checkNude == "undefined" || data.checkNude == false) return;
  ...
  const ress = await axios.get(`https://api.blacksky04.repl.co/imgurupload?link=${encodeURIComponent(dataa.url)}`)
  const link = ress.data.uploaded.image
  var res = await axios.get(`https://api.blacksky04.repl.co/checknude?key=wltOzZ4tD7&link=${link}`);
...
const ress = await axios.get(`https://api.blacksky04.repl.co/imgurupload?link=${encodeURIComponent(event.messageReply.attachments[0].url)}`)
  const link = ress.data.uploaded.image
  var res = await axios.get(`https://api.blacksky04.repl.co/checknude?key=wltOzZ4tD7&link=${link}`);
  var checkIMG = res.data.data
