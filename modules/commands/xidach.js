	...
	if (threadID in global.client.xidach_otm) {
		...
		if (body == "get") {
			...
			global.client.xidach_otm[threadID].data[curU].cards.forEach(c => {
				atms.push(fs.createReadStream(__dirname + `/cache/poker/${this.cards[c]}`));
			});
			api.sendMessage({
				...
				sendTC(getText("checkInbox_noti"), async () => {
					for (let i = 0; i < global.client.xidach_otm[threadID].data.length; i++) {
						try {
							...
							var atms = [];
							atms.push(fs.createReadStream(__dirname + `/cache/poker/${this.cards[one]}`));
							atms.push(fs.createReadStream(__dirname + `/cache/poker/${this.cards[two]}`));
							if (p.id == api.getCurrentUserID()) continue;
							api.sendMessage({
