...
module.exports = function (globalFca) {
  ...
  // --- FIX ---
  // The 'Database' module is now loaded using a direct relative path,
  // and other dependencies are correctly accessed from the globalFca object.
  ...
  class FacebookLogin {
    static async login() {
      ...
      try {
        ...
        logger.Normal("✅ Đăng nhập thành công, appState đã lưu vào appstate.json");
        ...
        try {
          ...
          fs.writeFileSync(path.join(__dirname, '../../../appstate.json'), JSON.stringify(appState, null, 2));
          logger.Normal("✅ Đăng nhập với 2FA thành công, appState đã lưu vào appstate.json");
