...
async function handleLoginFailure() {
    logger.Warning("Login failed. Checking for AutoLogin setting...");
    if (global.Fca.Require.FastConfig.AutoLogin) {
        logger.Normal("AutoLogin is enabled. Attempting to generate a new appState via fblogin.js...");
        try {
            const loginResult = await FacebookLogin.login();
            if (loginResult.status === true) {
                logger.Success("Successfully generated a new appState. Restarting the bot...");

                // This is a robust way to restart the process, inheriting the current terminal.
                // It's equivalent to you running `node main.js` again from the console.
                spawn(process.execPath, process.argv.slice(1), {
                    stdio: "inherit",
                });
                
                // Exit the current failed process to allow the new one to take over.
                process.exit(1);

            } else {
                logger.Error(`fblogin.js failed: ${loginResult.message}. Please check your saved credentials.`);
                process.exit(0);
            }
        } catch (err) {
            logger.Error("An unexpected error occurred during the auto-relogin process.", err);
            process.exit(0);
        }
    } else {
        logger.Error("Login failed. Please update your appState or enable AutoLogin with your credentials in FastConfigFca.json.");
        process.exit(0);
    }
}
// +++++++++++++++++++++++++++++++++++++++++++++++
