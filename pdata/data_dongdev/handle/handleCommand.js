module.exports = function ({ api, models, Users, Threads, Currencies }) {
    ...
    // Gửi message tự xóa
    async function sendTempMessage(api, msg, threadID, delay = 60, attachment = null, replyMsgID = null) {
...
if (typeof body === 'string' && body.startsWith(prefixBox)) {
    if (!ADMINBOT.includes(senderID) && configData.adminOnly === true)
        return sendTempMessage(
            api,
            `🚫 [ADMIN ONLY]\nXin lỗi, khu vực này chỉ dành cho các Admin quyền lực của bot. Nếu bạn là admin, hãy chứng minh đi nào!`,
            threadID, 12, null, messageID
        );
    if (!ADMINBOT.includes(senderID) && configData.adminPaseOnly === true)
        return sendTempMessage(
            api,
            `🚫 [ADMIN PASE ONLY]\nChỉ có Admin bot mới được phép dùng bot ở chat riêng! Bạn chưa đủ "level" để vào khu vực này đâu 😎`,
            threadID, 12, null, messageID
        );
    if (!ADMINBOT.includes(senderID) && configData.ndhOnly === true)
        return sendTempMessage(
            api,
            `🚫 [NDH ONLY]\nChỉ những người hỗ trợ bot (NDH) mới được dùng tính năng này!\nHãy liên hệ Admin nếu bạn muốn trở thành NDH nhé!`,
            threadID, 12, null, messageID
        );
}
        ...
        if (
            ...
            return sendTempMessage(
  api,
  `🔒 [QTV ONLY]\nƠ kìa! Lệnh này chỉ dành cho Quản Trị Viên nhóm thôi nha 🥲\nBạn chưa đủ "quyền lực" rồi, xin admin nhóm đi nào!`,
  threadID, 12, null, messageID
);
        }
        ...
        if (!prefixRegex.test(body)) {
            ...
            if (command && command.config) {
                if (command.config.usePrefix === false && commandName !== command.config.name)
                   return sendTempMessage(
  api,
  `❌ Có vẻ như bạn đã gõ nhầm lệnh.\n🔎 Lệnh chính xác phải là: "${command.config.name}"\nĐừng quên kiểm tra chính tả trước khi gửi nhé!`,
  threadID, 10, null, messageID
);
        ...
        if (!command) {
            ...
            fs.readdir(folderPath, (err, files) => {
                ...
                const msg = [
    `👋 Hey ${userName}!`,
    `⛔ Lệnh chưa có trong kho dữ liệu bot.${suggest ? `\n🌟 Gợi ý: Bạn thử "${suggest}" xem sao nhé!` : ""}`,
    latestFile ? `🎉 Mẹo: Lệnh mới nhất là: ${latestFile.name}` : "",
    `─────────────`,
    `🕒 Thời gian hiện tại: ${getTimeVN()}`,
    `💬 Tip: Dùng "menu all" để xem tất cả lệnh!`
].filter(Boolean).join('\n');
