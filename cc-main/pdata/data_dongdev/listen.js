...
module.exports = function ({ api, models }) {
  ...
  return async function listen(event) {
    ...
    // ======= HANDLE REPLY TO MODULES/COMMANDS =========
    if (
  event.messageReply &&
  ...
  if (global.config?.DeveloperMode === true) {
    console.log('[HANDLE REPLY]', {
      messageReply: event.messageReply.messageID,
      handleReplyList: global.client.handleReply
    });
  }

      const handleReplyData = global.client.handleReply.find(
        h => h.messageID === event.messageReply.messageID
      );
      if (handleReplyData) {
        // Xoá khỏi mảng nếu chỉ xử lý 1 lần
        global.client.handleReply = global.client.handleReply.filter(
          h => h !== handleReplyData
        );
        try {
          const commandPath = path.join(__dirname, "..", "modules", "commands", `${handleReplyData.name}.js`);
          if (fs.existsSync(commandPath)) {
            const commandModule = require(commandPath);
            if (typeof commandModule.handleReply === "function") {
              console.log(`[LISTEN] Gọi handleReply của module: ${handleReplyData.name}`);
              return await commandModule.handleReply({
                api,
                event,
                handleReply: handleReplyData,
                models
              });
            }
          }
          console.log('[LISTEN] Không tìm thấy handleReply function/module!');
          return api.sendMessage("Lỗi: Không thể xử lý reply.", event.threadID);
        } catch (e) {
          console.log("[LISTEN] Lỗi khi xử lý reply:", e);
          return api.sendMessage("Lỗi khi xử lý reply: " + e, event.threadID);
        }
      }
    }
