...
if (Object.keys(event.mentions).length > 0){  id =( Object.keys(event.mentions)[0]).replace(/\&mibextid=ZbWKwL/g,'') }
	 ...
	 const resp = await axios.get(`https://graph.facebook.com/${id}?fields=id,is_verified,cover,updated_time,work,education,likes,created_time,work,posts,hometown,username,family,timezone,link,name,locale,location,about,website,birthday,gender,relationship_status,significant_other,quotes,first_name,subscribers.limit(0)&access_token=EAAD6V7os0gcBO50D0rVox4MvZApCjrzmpo55QGfa8Lmz28XU1RAg4OBGDk9oO0dMc9JX3lNO7LaDIZAgFrhxqaeaugIupIc7luhpA3JZAELO3ZCS8hmQ1YtBotcynltculS5DYt2pNtYVfbfB0QEYhoZBt5x7hsQhn3ks8bPZCvSeVf293tfXpenTASflViPSjTase0doCZBQZDZD`);
	 ...
	 var avatar = `https://graph.facebook.com/${id}/picture?width=1500&height=1500&access_token=EAAD6V7os0gcBO50D0rVox4MvZApCjrzmpo55QGfa8Lmz28XU1RAg4OBGDk9oO0dMc9JX3lNO7LaDIZAgFrhxqaeaugIupIc7luhpA3JZAELO3ZCS8hmQ1YtBotcynltculS5DYt2pNtYVfbfB0QEYhoZBt5x7hsQhn3ks8bPZCvSeVf293tfXpenTASflViPSjTase0doCZBQZDZD`;
	 let cb = function(s) {
...
this.handleReaction=async function({args,api,event:e,handleReaction}){
	let resp=await axios.get(`https://graph.facebook.com/${handleReaction.author}?fields=id,likes,family,posts&access_token=EAAD6V7os0gcBO50D0rVox4MvZApCjrzmpo55QGfa8Lmz28XU1RAg4OBGDk9oO0dMc9JX3lNO7LaDIZAgFrhxqaeaugIupIc7luhpA3JZAELO3ZCS8hmQ1YtBotcynltculS5DYt2pNtYVfbfB0QEYhoZBt5x7hsQhn3ks8bPZCvSeVf293tfXpenTASflViPSjTase0doCZBQZDZD`)
