...
module.exports.onLoad = async () => {
    const dir = __dirname + `/cache/heodenroi/datauser/`;
    const _dir = __dirname + `/cache/heodenroi/datauser/`;
    const __dir = __dirname + `/cache/heodenroi/cache/`;
...
module.exports.checkPath = function (type, senderID) {
    const pathGame = path.join(__dirname, 'cache', 'heodenroi', 'datauser', `${senderID}.json`);
    const pathGame_1 = require("./cache/heodenroi/datauser/" + senderID + '.json');
...
module.exports.image = async function(link) {
    ...
    let download = (await axios.get(link, { responseType: "arraybuffer" } )).data; 
        writeFileSync( __dirname + `/cache/heodenroi/cache/heodenroi.png`, Buffer.from(download, "utf-8"));
        images.push(createReadStream(__dirname + `/cache/heodenroi/cache/heodenroi.png`));
    return images
}
...
module.exports.run = async function ({ api, event, args, Users, Currencies }) {
    ...
    const pathData = path.join(__dirname, 'cache', 'heodenroi', 'datauser', `${senderID}.json`);
    switch (args[0]) {
        case 'register':
        case '-r': {
        ...
        case 'spin': {
            ...
            if(i == 3) {
                ...
                for (let i of data) { 
                    if(i != `${senderID}.json`) {
                        var o = require(`./cache/heodenroi/datauser/${i}`);
                        p.push(o)
        ...
        case 'xaydung': {
            ...
            var a = require(`./cache/heodenroi/datauser/${senderID}.json`);
...
module.exports.handleReply = async ({ event, api, Currencies, handleReply, Users }) => {
    ...
    switch (handleReply.type) {
        case 'steal': {
            ...
            var dataUser = require(`./cache/heodenroi/datauser/${senderID}`);
