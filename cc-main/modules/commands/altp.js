...
const questionsURL = (global.thanhdz && global.thanhdz.altp_url) ? global.thanhdz.altp_url : "";
...
module.exports.onLoad = async ({}) => {
	...
	if (!global.hasOwnProperty('_data_altp')) global._data_altp = [];
	if (!questionsURL || typeof questionsURL !== "string" || !/^https?:\/\//.test(questionsURL)) {
	...
	try {
		...
		if (Array.isArray(res.data)) {
			...
			global._loadFailed_altp = false;
		} else {
			throw new Error("Dữ liệu câu hỏi không đúng định dạng mảng.");
		}
	} catch (e) {
		console.error("[altp] Lỗi tải câu hỏi:", e);
		global._loadFailed_altp = true;
	}
};
...
module.exports.run = async ({ api, event, args, Users, Currencies }) => {
	...
	if (global._loadFailed_altp == true || global._data_altp.length == 0) {
		send("Lệnh này hiện không khả dụng, thử lại trong chút lát...", threadID, messageID);
	} else {
		try {
			switch(query) {
				case "play":
					if (global._isPlaying_altp.has(senderID)) {
						send("Bạn đang ở trong phần chơi của mình, không thể bắt đầu phần chơi khác", threadID, messageID);
					} else {
						const userMoney = (await Currencies.getData(senderID)).money || null;
						if (userMoney == null) {
							send("Tài khoản của bạn không có trong hệ thống tiền tệ, vui lòng thử lại sau...", threadID, messageID);
						} else if (userMoney < _play_fee) {
							send(`Bạn cần ${_play_fee}$ để tham gia...`, threadID, messageID);
						} else {
				...
				case "help":
					if (!global._isPlaying_altp.has(senderID)) {
						send("Bạn đang không có phần chơi nào cả...", threadID, messageID);
					} else {
						const userData = global._isPlaying_altp.get(senderID);
				...
				case "bxh":
					...
					if (allUsersDatas.length == 0) {
						send("Chưa có dữ liệu để thống kê...", threadID, messageID);
					} else {
			...
			if (query != "bxh") {
				...
				send("Đã có lỗi xảy ra, đang hoàn trả tiền...", threadID, messageID);
			}
			console.error(e);
		}
	}
...
module.exports.handleReply = async function({ api, event, Currencies, Users, handleReply }) {
	const send = api.sendMessage;
	...
	if (author == senderID && choice != null) {
		const userData = {...global._isPlaying_altp.get(senderID)};
		if (!userData.hasOwnProperty("index")) return;
		global.client.handleReply.splice(global.client.handleReply.findIndex(e => e.messageID == handleReply.messageID), 1);
			...
			if (parseInt(choice) > help_available_count || parseInt(choice) < 1) {
				...
				if (_help_chosen_property == "help_1") {
					...
					const newQuestion = shuffle([...global._data_altp])
											.filter(e => questions.every(ee => ee.question != e.question))[0];
