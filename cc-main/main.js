...
const { autoLogin } = require(path.join(__dirname, "pdata", "autologin"));

async function loginAppstate() {
  logger("Bắt đầu login appstate...", "LOGIN-APPSTATE");
  autoLogin((err, api) => {
    if (err) {
      logger("Không thể login bằng mật khẩu, thay appstate hoặc mật khẩu!", "[ LOGIN-ERROR ]");
      process.exit(0);
    }
    logger("Đăng nhập thành công, khởi động lại!", "[ LOGIN-ACCOUNT ]");
    if (typeof uptime === "function") uptime();
  });
}
module.exports = loginAppstate;
